...
scp -i "$SSH_KEY" "$LOCAL_CONF" "$EC2_USER@$EC2_HOST:/home/$EC2_USER/nginx-api.conf"
...
ssh -i "$SSH_KEY" "$EC2_USER@$EC2_HOST" << 'EOF'
    ...
    sudo mv ~/nginx-api.conf /etc/nginx/sites-available/hqmx-api
    
    echo "   Enabling site..."
    sudo ln -sf /etc/nginx/sites-available/hqmx-api /etc/nginx/sites-enabled/
