...
## 🛑 Git 작업 필수 가이드 (무한 로딩 및 락 방지)

**증상**: VS Code나 에이전트 작업 중 "무한 로딩"이나 Git 프로세스가 멈추는 현상.
**원인**: 중첩된 Git 구조(Nested Repositories)에서 부모/자식 리포지토리 간의 Index Lock 충돌 및 커밋 메시지 대기(Hanging Commit) 현상.

### ✅ 필수 작업 수칙 (Action Items)

1.  **Bottom-Up 커밋 원칙 준수**:
    *   반드시 **자식 리포지토리(`generator`, `converter` 등)**를 먼저 커밋/푸시합니다.
    *   그 후 **부모 리포지토리(`hqmx`)**를 커밋/푸시합니다.
    *   순서가 꼬이면 부모 리포지토리가 자식의 변경을 계속 감지하여 무한 루프에 빠질 수 있습니다.

2.  **커밋 메시지 플래그 필수 (`-m`)**:
    *   터미널에서 커밋 시 **반드시 `-m "메시지"` 옵션을 사용**하세요.
    *   `git commit`만 입력하면 에디터가 백그라운드에서 열려 사용자 입력을 무한히 기다리는 "좀비 프로세스"가 됩니다.
    *   (O) `git commit -m "update"`
    *   (X) `git commit`

3.  **프로세스 강제 종료 (비상시)**:
    *   무한 로딩 발생 시: `ps aux | grep git` 으로 멈춘 프로세스 확인 후 `kill -9 [PID]`로 종료.
    *   락 파일 제거: `rm .git/index.lock` (각 리포지토리 확인)
